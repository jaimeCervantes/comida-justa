...
- **Parte la corrida de Playwright en shards en cuanto pase de ~20 escenarios**, y lánzalos uno detrás de otro. **El número de tramos sale del recuento, no es fijo**: cuenta primero con `pnpm exec playwright test <rutas> --list` y divide para que cada tramo tenga **~15 escenarios como mucho** —79 escenarios en tres tramos se pasan del límite; en cinco caben holgados—. Cada tramo paga además ~40 s de arranque del servidor, que no se reparten. **Nunca en paralelo**: cada tramo levanta su propio servidor en el mismo puerto. Y **borra `.next` antes de cada tramo** (`rm -rf .next`): dos servidores de desarrollo consecutivos compartiendo esa carpeta la dejan a medias, y el síntoma es que rutas que existen empiezan a responder **404** cayendo al catch-all `[slug]` — se diagnostica como un fallo del código y no lo es. Comprobado: el primer tramo tras limpiar pasa siempre y el siguiente cae; limpiando antes de cada uno, 79/79. El motivo de partir no es la velocidad: una corrida que se corta a la mitad —por un tiempo de espera o un `Ctrl+C`— deja sin ejecutar sus `afterEach`, y el residuo en la base compartida hace fallar la corrida siguiente con errores que no tienen nada que ver (404 en rutas que sí existen, tiendas duplicadas). **Un fallo justo después de una corrida cortada es residuo hasta que se demuestre lo contrario**: mata el `next dev` huérfano que se quedó escuchando en el puerto, deja que el barrido de `globalTeardown` limpie, y repite antes de diagnosticar nada.
