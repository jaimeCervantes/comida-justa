import { ISearchPostRepository } from "~/use_cases/searchPosts/ports/ISearchPostRepository";
import { collections } from "~/infra/dataAccess/postUtils";
import { FirestorePost } from "~/infra/dataAccess/Posts";
import { ISearchPostResultDTO } from "~/use_cases/searchPosts/dtos/ISearchPostResultDTO";
import VertexEmbeddingService from "~/infra/services/VertexEmbeddingService";
...
export class FirebaseSearchPostRepository implements ISearchPostRepository {
  async search(
    ...
    try {
      ...
      try {
        const cacheRef = (postsRef as any).firestore
          .collection("search_embedding")
          .doc(cacheDocId);
        ...
        if (!vector) {
          ...
          cacheRef
            .set({ query: normalizedQuery, vector, createdAt: new Date() })
            .catch((err: any) =>
        ...
        console.warn(
          "Embedding cache read/write failed, continuing without cache:",
          cacheErr,
        );
        vector = await embeddingService.generateEmbedding(query);
      }
