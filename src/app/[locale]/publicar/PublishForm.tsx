...
export default function PublishForm({
  ...
  /**
   * Qué campos rechazó el navegador, escuchando el evento `invalid` de cada uno.
   *
   * `checkValidity()` —el que dispara `Form` al enviar— lanza un `invalid` **por control
   * rechazado**, y React lo propaga por su árbol. Es la vía fiable: leer `form.elements` desde
   * fuera dependía de un momento que en el navegador no se comportaba como en jsdom, y el
   * asistente se quedaba sin saltar al paso ni marcar nada — comprobado, el foco ni se movía.
   *
   * Se acumulan en un `ref` porque llegan en ráfaga —uno por campo, en el mismo tick— y se
   * resuelven al final con un `setTimeout(0)`: para entonces ya están todos.
   */
