...
describe("SearchPostsUseCase", () => {
    ...
    it("should call repository search with correct parameters", async () => {
        const mockResults: ISearchPostResultDTO[] = [
            { id: "1", title: "Test Post", content: "Content", slug: "test-post", media: { url: "", type: "image" }, user: { id: "u1" }, createdAt: new Date(), contactInfo: { phone: "123" } }
        ];
        mockRepository.search.mockResolvedValue({ results: mockResults, total: 1 });

        const result = await useCase.execute({ query: "test", page: 1, pageSize: 10 });

        expect(mockRepository.search).toHaveBeenCalledWith("test", 1, 10);
