/**
 * La semilla del catálogo de prácticas: los cuatro pilares y las prácticas de Sueño.
 *
 * **Es contenido, no esquema.** Las tablas las creó Alembic
 * (`0049_2026-09-04_add_practice_catalog.py`); esto las llena, y es idempotente.
 *
 * Las trece prácticas de Sueño salen de deduplicar las veintidós apariciones que hoy tiene la misma
 * acción repartidas entre el ritual, el santuario, la descarga mental y el catálogo. Esa
 * deduplicación es un juicio editorial, no un `diff`: por eso vive aquí, en filas revisables una por
 * una, y no en un script que intente adivinarla.
 *
 * **Los DOIs ligados a cada práctica no se inventaron.** Salen de dos sitios: de los comentarios de
 * `references.ts`, que ya explicaban qué afirmación sostenía cada estudio añadido, y del título que
 * devuelve Crossref cuando dice sin ambigüedad de qué trata. Una práctica sin estudio se queda sin
 * estudio —la infusión y el notar la claridad no tienen ninguno de los 116—, porque una bibliografía
 * de adorno es peor que una lista corta.
 */
...
export const SLEEP_PRACTICE_SEED: readonly PracticeSeed[] = [
  ...
  {
    key: "sleep-slow-breathing",
    effortMinutes: 5,
    costLevel: 0,
    // La práctica compartida que prueba el modelo: es de Mente en portada y de Sueño también.
    // Hasta ahora estaba escrita dos veces y las dos versiones se contradecían.
    ...
    es: {
      ...
      howTo:
        "Baja las respiraciones por minuto y aflójate. No persigas una proporción exacta entre inhalar y exhalar: el ensayo que la probó, y su réplica, no hallaron diferencia, y perseguirla es la excusa habitual para abandonar creyendo que se hace mal.",
    },
    en: {
      ...
      howTo:
        "Bring your breaths per minute down and loosen up. Do not chase an exact ratio between inhaling and exhaling: the trial that tested it, and its replication, found no difference, and chasing it is the usual excuse for quitting in the belief you are doing it wrong.",
    },
  },
  {
